      ...
      <div className="container mx-auto px-6 py-12">
        <div className="grid md:grid-cols-3 gap-8 items-center">
          ...
          <div className="flex justify-center gap-6">
            {socialLinks.map((link, index) => <motion.a key={index} href={link.href} target="_blank" rel="noopener noreferrer" whileHover={{
            scale: 1.1
          }} whileTap={{
            scale: 0.95
          }} className="p-3 rounded-full bg-secondary hover:bg-primary/20 transition-all duration-300 hover:shadow-glow-primary" aria-label={link.label}>
        ...
        <div className="border-t border-border mt-8 pt-8 text-center text-sm text-muted-foreground">
          <p>ALL RIGHTS AND RESERVED OWNED BY MOHAMED RASAL©
2025</p>
        </div>
      </div>
    </footer>;
